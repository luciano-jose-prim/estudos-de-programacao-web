<?php
 ...
 //antes de continuarmos o cadastro, vamos nos certificar que não existe nenhum outro usuário com este mesmo login já cadastrado no banco de dados. Verificação semelhante pode ser feita em relação ao campo do e-mail

 $sql = "SELECT login, email FROM $nomeDaTabela WHERE login = '$loginCripto' OR email = '$email'";

 $conexao->query($sql);

 //testando a condição de erro para o cadastro. Se o PHP receber do MySQL o valor 1 para a consulta anterior, significa que o MySQL já encontrou, cadastrados, no banco de dados, outro usuário com o mesmo login ou e-mail do usuário atual
